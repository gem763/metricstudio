...
from src.notebook_experiment_utils import summarize_vs_benchmark, overall_pivot
from src.pattern import AmountSurge, Bollinger, Disparity, High, MFI, Pattern, Trending
...
def _build_trend_pattern(name: str) -> Pattern:
...
def _build_disparity_candidate(spec: dict[str, object]) -> Pattern:
    pattern: Pattern = Disparity(window=20, name=spec["name"]).on(
