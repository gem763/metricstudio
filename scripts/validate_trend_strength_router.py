...
from src.pattern import AmountSurge, Bollinger, High, MFI, Pattern, RetestBreakout, Trending
...
def _variant_patterns(regime: Regime) -> list[Pattern]:
    return [
...
def build_regime_gap_table(bt: Backtest) -> pd.DataFrame:
    ...
    for regime_label, regime_kind in REGIME_SPECS:
        ...
        stats = bt.analyze(Pattern(name="benchmark").when(regime), *_variant_patterns(regime))
...
def _build_router_patterns() -> list[Pattern]:
