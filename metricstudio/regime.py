"""Market regime frame builders and composable regime selectors."""
...
def _normalize_regime_kind(kind: str) -> str:
    """
    사용자 입력을 내부 regime kind 키로 정규화한다.
    """
...
def _positive_numeric_series(series: pd.Series) -> pd.Series:
    """
    시계열을 숫자로 변환하고 0 이하 값은 결측으로 정리한다.
    """
...
def _positive_numeric_frame(frame: pd.DataFrame) -> pd.DataFrame:
    """
    표 전체를 숫자로 변환하고 0 이하 값은 결측으로 정리한다.
    """
...
def _rolling_last_percentile_rank(
    ...
    """
    롤링 윈도우 내 마지막 값의 분위 순위를 0~1 비율로 계산한다.
    """
...
def build_market_cap_bucket_masks(
    ...
    """
    시가총액 누적 비중 기준으로 large/mid/small 마스크를 만든다.
    """
...
def _ratio_with_mask(
    ...
    """
    유효 종목 집합 안에서 조건 충족 비율을 일자별로 계산한다.
    """
...
def build_regime_frame(
    ...
    """
    시장 breadth/volatility 지표를 조합해 regime 판별용 테이블을 만든다.

    반환값에는 현재 사용하는 4개 regime boolean 컬럼과 label, 그리고
    과거 실험 비교를 위한 legacy bucket 컬럼도 함께 담는다.
    """

    market_close = _positive_numeric_series(market_close)
    close_df = _positive_numeric_frame(close_df)
    amount_df = _positive_numeric_frame(amount_df)
    ...
    # 아래 legacy_* 컬럼은 예전 4분류 실험과 비교할 때만 쓴다.
...
class Regime:
    """
    Backtest가 계산한 regime mask를 조합식으로 표현하는 불변에 가까운 셀렉터.
    """
    ...
    def on(
        ...
        """
        leaf regime을 특정 market/kind 조합으로 설정한다.
        """
    ...
    def _iter_leaf_regimes(self):
        """
        합성 트리 안의 leaf regime들을 순서대로 순회한다.
        """
    ...
    def cache_key(self):
        """
        동일 regime 식 재사용을 위한 구조적 캐시 키를 반환한다.
        """
    ...
    def _bind(self, dates: np.ndarray, mask_values: np.ndarray, frame: pd.DataFrame) -> None:
        """
        Backtest가 계산한 leaf mask와 원본 frame을 leaf regime에 바인딩한다.
        """
    ...
    def mask(self, length: int | None = None) -> np.ndarray:
        """
        leaf/합성 regime 식을 최종 bool mask로 계산한다.
        """
    ...
    def to_frame(self, copy: bool = True) -> pd.DataFrame:
        """
        leaf regime이 공유하는 원본 regime frame을 반환한다.
        """
